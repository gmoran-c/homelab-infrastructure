...
| Servicio / flujo | Transporte y puerto | Desde Internet | Desde VPN (`10.10.10.0/24`) | Alcance esperado |
|---|---|---|---|---|
| nginx / web | TCP 80 | Publicado; redirige a HTTPS | Accesible si la regla lo permite | Único frontend publicado |
| nginx / HTTPS | TCP 443 | Publicado; certificado autofirmado temporal | Accesible por `10.10.10.1` | Entrada web y `/api/` |
...
- Revisar el NAT y las excepciones del router en la misma interfaz WAN (`ppp0.1`).
